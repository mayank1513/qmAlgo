...
main()
{
    int t,k,k1,i,i1,ifd,if1,j,x,x1,y,z,a,b,b1,c,h2;
    int *fd,*f,*h1;
    unsigned *F,*F1,n,N,M1,M2;
    unsigned ****G,**gc,**B,**B1,***tick;
    unsigned **T,**T1,**Tf;
   // unsigned **RowGray,**ColGray;
    char file[15];
    FILE *fp,*fp1;
...
//**************************************************************************Allocating Memory to
    ...
    printf("\nname of input file(less than 15 laters, including'.txt')\n");
    scanf("%s",file);
    printf("\n%s",file);
    //file="Bullionexp.txt";
    fp=fopen(file,"r");//###################################################opening input file
...
printf("if1=%d",if1);
   for(j=0;j<if1;j++)//i,i1,y->;j,k|^__Elemination
    ...
    h1=(int *)calloc(z,sizeof(int));
            if(h1==NULL) printf("\nh1\tsufficient memory not avilable!!\n");
   ...
   // mergesort(h1,0,z-1);
    ...
    for(a=0;a<i-y;a++)
    {
        ...
        for(j=n-1;j>=0;j--)
        {
            printf("\n%d,%d",T1[F1[a]][j],F1[a]);
            if(T1[F1[a]][j]==0)
                fprintf(fp1,"%c'",'a'+n-1-j);
            else if(T1[F1[a]][j]==1)
                fprintf(fp1,"%c",'a'+n-1-j);
        }
    }

    /*M1=pow(2,n/2); M2=pow(2,(n-n/2));

    printf("Hari 1!!");

    RowGray=(int **)malloc(M1*sizeof(int *));
    if(RowGray==NULL) printf("\nRowGray\tsufficient memory not avilable!!\n");

    for(i=0;i<M1;i++)
    {
        RowGray[i]=(int *)calloc(n,sizeof(int));
        if(RowGray[i]==NULL) printf("\nRow Gray\tsufficient memory not avilable!!\n");
    }

    ColGray=(int **)malloc(M2*sizeof(int *));
    if(ColGray==NULL) printf("\nColGray\tsufficient memory not avilable!!\n");

    for(i=0;i<M2;i++)
    {
        ColGray[i]=(int *)calloc(n,sizeof(int));
        if(ColGray[i]==NULL) printf("\nColGray\tsufficient memory not avilable!!\n");
    }
